...
<Note>
  The CLI verb set is now complete: object reads and writes, snapshot
  and fork management, signed URLs, and a stdio Model Context Protocol
  server. Pair any of the verbs with `--adapter <name>` to pick the
  backend; or set `STORAGE_ADAPTER` once and let every command read it.
</Note>
...
- **AI agents:** [`mcp`](/cli/mcp) boots a stdio Model Context Protocol server so Claude Desktop, Cursor, or any MCP host can drive every verb.
