...
title: Registry
eyebrow: Adapters
description: Pick adapters by name at runtime — ADAPTERS, buildAdapter, getAdapterEnvVars.
section: adapters
---
...
`buildAdapter('tigris')` is the only call that pulls `@tigrisdata/storage` into your runtime. The other 17 adapters' peer-SDK packages — `@aws-sdk/client-s3`, `@azure/storage-blob`, `@google-cloud/storage`, `@octokit/rest`, and so on — are never imported.
...
Only the one adapter and its peer SDK end up in your bundle — the registry adds nothing, the other 17 adapters tree-shake out.
