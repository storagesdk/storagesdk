...
- **CLI.** [`@storagesdk/cli`](./packages/cli/README.md) ships the `storage` and `storagesdk` binaries. Read any backend (`ls`, `stat`, `cat`, `sign`) and inspect snapshots/forks (`storage snapshots`, `storage forks`) — with `--snapshot`/`--fork` to scope reads. `storage adapters` discovers every adapter with its env-var spec including backend-native fallbacks. Writes and an `mcp` subcommand follow.
