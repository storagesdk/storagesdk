...
export function storageAdapterTestSuite<Raw = unknown>(
  ...
  d(opts.name, describeOpts, () => {
    ...
    describe('forks.merge / forks.rebase', {
      ...
      it('merge preserves a parent-side edit when the fork left the path alone', async () => {
        // Parent edits a file the fork never touched. Fork still
        // carries the base version. Merge must NOT clobber the
        // parent's edit with the fork's stale copy — a naive
        // "any etag mismatch = modified" classifier gets this wrong.
        await ctx.upload('shared.txt', 'base version');
        const snap = await ctx.snapshots.create();
        const name = ctx.forkName('mg-edit');
        await ctx.forks.create({ name, fromSnapshot: snap.id });

        // Parent edits post-fork. Fork does not touch shared.txt.
        await ctx.upload('shared.txt', 'parent edit');

        const result = await ctx.forks.merge?.(name);
        expect(result).toBeDefined();
        if (!result) return;

        expect(bodyText(await ctx.download('shared.txt'))).toBe('parent edit');
      ...
      it('diff (ahead) reports paths present on fork but not parent', async () => {
        await ctx.upload('shared.txt', 'shared');
        const snap = await ctx.snapshots.create();
      ...
      it('diff (behind) reports parent-only additions and parent-side deletions', async () => {
        await ctx.upload('shared.txt', 'shared');
