...
A unified TypeScript SDK for storage with first-class support for snapshotting, forking across Tigris, Amazon S3, Cloudflare R2, Archil, Code Storage, Mesa, GCS, Azure Blob, Vercel Blob and many more.
...
- **CLI.** [`@storagesdk/cli`](./packages/cli/README.md) ships the `storage` and `storagesdk` binaries with the full verb set: reads (`ls`, `stat`, `cat`), writes (`cp`, `mv`, `rm`), signed URLs (`sign download`, `sign upload`), snapshot/fork list and management (`storage snapshots`, `storage forks`, `storage snapshot {create,rm}`, `storage fork {create,rm}`), and a stdio Model Context Protocol server (`storage mcp`) for Claude Desktop / Cursor / any MCP host. `cp` and `mv` use a `storage://` URL scheme to mark remote paths; `-` is stdin/stdout. `--fork`/`--snapshot` scope operations into derived state. `storage adapters` discovers every adapter with its env-var spec including backend-native fallbacks.
...
| Code Storage | [`@storagesdk/adapters/code-storage`](./packages/adapters/src/code-storage/README.md) | [Code Storage](https://code.storage/) repositories — snapshots are tags, forks are branches. |
...
| Mesa | [`@storagesdk/adapters/mesa`](./packages/adapters/src/mesa/README.md) | [Mesa](https://mesa.dev/) repositories — snapshots and forks are bookmarks. |
