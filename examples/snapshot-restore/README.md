...
6. **`storage.snapshots.delete(id)`** — clean up the snapshot location. On Tigris this throws `NotSupported` because snapshots there are point-in-time *references* to bucket state, not separate copies — the example catches that case and prints a note.
